...
if( isset($accion) ){
   ...
   // $fields['id'] = filter_input(INPUT_POST, 'id');
    ...
    if($accion == "new"){
        try {
            database::queryInsert("resultado_temp", $fields);

        }catch(Exception $e){
            $error = true;
            header("Location: resultados.php?alert=success&info=".urlencode($e->getMessage()));
        }
        if( !$error ){
            header("Location: resultados.php?alert=primary&info=Registro añadido correstamente.");
        ...
        try {
            database::queryUpdate("resultado_temp", $fields, $key_fields);
            database::queryUpdate("resultado_db", $fields, $key_fields);
        }catch(Exception $e){
            $error = true;
            header("Location: resultados.php?alert=primary&info=".urlencode($e->getMessage()));
        }
        if( !$error ){
            header("Location: resultados.php?alert=success&info=Registro modificado correctamente.");
        }
    }else{
        header("Location: resultados.php?alert=primary&info=Error al realizar la acción.");
    }
}
...
if( isset($id) ){
    ...
    $result = $database->query("SELECT * FROM resultado_temp WHERE id = '".$id."'");
    if ($result->num_rows == 1) {
        $row = $result->fetch_array();
    } else {
        header("Location: resultados.php?alert=primary&info=Error al identificar el registro.");
    }
}
...
<?php ini(); ?>
    ...
    <form data-abide action="publicacion.php" method="post">
        ...
        <div style="text-align: center;">
            ...
            <?php } ?>
                ...
                <a class="btn btn-danger" href="resultados.php"><i class="fa fa-times"></i>Cancelar</a>
