...
if( isset($accion) ){
    ...
    if($accion=="mod"){
        $key_fields = array();
        $key_fields['id_dblp'] = filter_input(INPUT_POST, "id");
        try {
            database::queryUpdate("dblp_results", $fields, $key_fields);
        }catch(Exception $e){
            $error = true;
...
if( isset($id) ){
    ...
    $result = $database->query("SELECT * FROM dblp_results WHERE id_dblp = '".$id."'");
...
<?php ini(); ?>
    ...
    <form data-abide action="publicacion_dblp.php" method="post">
        <img class="d-block mx-auto mb-4" src="../imagenes/logo_fusion.jpg" alt="" width="25%" height="25%">
        <p class="display-6 fw-bold">Datos de la publicación: <?php if(isset($row["id_dblp"])){echo $row["id_dblp"];} ?></p>
        <div class="col-lg-6 mx-auto">
            <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
                <input type="hidden" name="accion" value="<?php echo (isset($row["id_dblp"]))?"mod":"new"; ?>" />
        ...
        <div class="input-group mb-3">
            ...
            <input type="text" readonly class="form-control" name="id" required="required" placeholder="" <?php if(isset($row["id_dblp"])){echo 'value="'.$row["id_dblp"].'"';} ?> />
